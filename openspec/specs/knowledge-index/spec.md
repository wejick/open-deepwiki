...
During OKF bundle ingestion, the system SHALL derive concept edges from markdown links between concepts, resolving bundle-absolute (`/path.md`), repo-root-absolute (`/openwiki/path.md` — the form openwiki itself writes), and relative link forms, and SHALL store them in an `edges` table (`from_id`, `to_id`). Where an absolute link is ambiguous, the bundle-absolute reading SHALL win, so a bundle that genuinely contains a concept at that path is never misresolved. Self-links, duplicate edges, and links that do not resolve to an indexed concept SHALL be silently dropped; edges are advisory.
...
#### Scenario: Repo-root-absolute link resolves
- **WHEN** a concept links `[a](/openwiki/workflows/generation.md)` and the bundle contains the concept `workflows/generation`
- **THEN** an edge is stored to `workflows/generation`, because the bundle directory is a path prefix rather than part of the concept id

#### Scenario: Prefix is not stripped when it would invent a target
- **WHEN** a concept links `[a](/openwiki/missing.md)` and the bundle contains neither `openwiki/missing` nor `missing`
- **THEN** no edge is stored
